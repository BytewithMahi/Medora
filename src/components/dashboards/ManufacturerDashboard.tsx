                    ...
                    >
                        ...
                        <div className="glassmorphism-dark p-6 rounded-3xl flex flex-col items-center justify-center text-center relative overflow-hidden group">
                            ...
                            <div className={`w-40 h-40 bg-white rounded-xl flex items-center justify-center mb-6 relative z-10 transition-all duration-700 ${isGenerated ? 'scale-100 opacity-100' : 'scale-90 opacity-10 filter blur-sm'}`}>
                                {isGenerated ? (
                                    <div className="w-36 h-36 bg-[url('https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=medchain:batch:ax792')] bg-cover opacity-90 mix-blend-multiply" />
                                ) : (
                                    <QrCode className="w-16 h-16 text-black/20" />
                                )}
                            </div>

                            <button disabled={!isGenerated} className={`relative z-10 px-6 py-2 rounded-full font-semibold transition-all ${isGenerated ? 'bg-white/10 text-white hover:bg-white/20 border border-white/20 hover:scale-105' : 'bg-white/5 text-white/30 cursor-not-allowed'}`}>
                                Download QR PDF
                            </button>
                        </div>
